...
class FleetHealthCheck:
    def __init__(self, fc: str):
        ...
        try:
            response = requests.get("https://atzptl.devices.a2z.com", auth=HTTPKerberosAuth(mutual_authentication=OPTIONAL), verify=False, cookies=self.cookie)
            if response.status_code != 200:
                self.reset_mw_cookie()
                response = requests.get("https://atzptl.devices.a2z.com", auth=HTTPKerberosAuth(mutual_authentication=OPTIONAL), verify=False, cookies=self.cookie)
            print("Authenticated to AZPTL portal")
        except Exception as e:
            print("Failed to authenticate to AZPTL portal")
    ...
    def get_health_report(self):
        ...
        """
        url = "https://atzptl.devices.a2z.com/api/gethealthmetrics?warehouseID=GYR1"
        tries = 1
        while tries < 4:
            try:
                response = requests.get(url, auth=HTTPKerberosAuth(mutual_authentication=OPTIONAL), verify=False, cookies=self.cookie)
                if response.status_code == 200:
                    return response.text
                else:
                    print(f"Failed to get data for {self.fc}")
                    self.reset_mw_cookie()
                    tries += 1
            except Exception as e:
                print(f"Failed to get data for {self.fc}")
