...
try:
    resp = req.get(url, auth=HTTPKerberosAuth(mutual_authentication=OPTIONAL), verify=False, allow_redirects=True)
    if resp.status_code != 200:
        send_email("There are no offline pods")
        pass
    podDF = pd.read_csv(StringIO(resp.text), sep=',').drop(['xValue','Index','Kiva_System_Key','Building','Pod_Recipe_Family'], axis=1)
    formathtml = """
    <html>
        <style>
            h1 {
    ...
    text = 'Error in {f} | Code: {c} | {m}'.format(f=os.path.basename(__file__), c=type(e).__name__, m=str(e))
