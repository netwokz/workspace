...
class Symbol:
    ...
    def draw(self, position):
        ...
        self.font_increase = self.font_size + \
            self.font_increase_speed if 0 < self.y < HEIGHT else 0
        font = pg.font.Font(
            'C:\\Users\\netwokz\\Documents\\code\\font\\MS Mincho.ttf', self.font_increase)
        ...
        if position < 8:
            self.char = font.render(
                self.value, True, (255 - 32 * position, 255, 255 - 32 * position))
        if position >= 8:
            self.char = font.render(
                self.value, True, (0, 255 - 22 * (position - 8), 0))
...
class SymbolColumn:
    def __init__(self, x, y, font_size):
        ...
        self.font_size = font_size
        self.symbols = [Symbol(x=x, y=i, font_size=self.font_size, speed=self.speed) for i in range(
            y, y - (self.font_size + 10) * self.column_height, - (self.font_size + 10))]

    def draw(self):
        [symbol.draw(position=i) for i, symbol in enumerate(self.symbols)]


symbol_columns = [SymbolColumn(x=i, y=0, font_size=20)
                  for i in range(100, WIDTH - 100, 200)]
